...
def load_and_split_pdb(pdb_path):
    """
    Load a PDB file with multiple models using PandasPdb and split it into a dictionary of PandasPdb objects,
    one for each model.
    """
    ppdb = PandasPdb()
    ppdb.read_pdb(pdb_path)

    models = ppdb.get_model_start_end()
    models.model_idx = models.model_idx.astype(str).str.strip()
    model_dict = {}

    for i, model in models.iterrows():
        mask = (ppdb.df["ATOM"].line_idx.values > model.start_idx) & (
            ppdb.df["ATOM"].line_idx.values < model.end_idx
        )
        model_dict[model.model_idx] = ppdb.df["ATOM"].loc[mask].copy().reset_index(drop=True)
    return model_dict
...
def codnas_to_pyg(cluster_id, cluster_info, download_dir):
    """
    ...
    pdb_ids = set([member.split("_")[0] for member in cluster_members])
    fasta = read_fasta(pjoin(download_dir, cluster_id, f"{cluster_id}_aligned.fasta"))
    fasta_non_members = read_fasta(pjoin(download_dir, cluster_id, f"{cluster_id}_non_members.fasta"))
    pdb = load_and_split_pdb(pjoin(download_dir, cluster_id, f"{cluster_id}.pdb"))

    pyg_dict = {}
    for k in pdb_ids:
        pyg_dict[k] = protein_to_pyg(df=pdb[k], remove_nonstandard=False)
        pyg_dict[k].coords = pyg_dict[k].coords[:, :3]
        fasta_sequences = {fasta_key: fasta_seq for fasta_key, fasta_seq in fasta.items() if k == fasta_key.split('_')[0]}
        fasta_non_members_sequences = {
            fasta_key: fasta_seq for fasta_key, fasta_seq in fasta_non_members.items() if k == fasta_key.split('_')[0]
        }
        pyg_dict[k] = align_chains_of_pdb(
            protein=pyg_dict[k],
            aligned_sequences=fasta_sequences,
            sequences_non_members=fasta_non_members_sequences,
            clusters_dict=cluster_info,
        )
