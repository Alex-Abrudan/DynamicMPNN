"""
Stage 3 of the processing pipeline for mmseqs2 80% sequence clusters.

Reads per-cluster CIFs + aligned FASTAs from pdb_cache_seq80/
(produced by gather_sequences_seq80.py) and featurises each cluster
into a .pt file → train_pt_multi_chain/{cluster80_id}.pt

The cluster80 IDs (integers) match the .pt filenames used by the
datamodule when cluster_sampling_csv references them.

Usage:
    python process_pt_seq80.py [--chunk_idx N --n_chunks 200] [--n_proc 16]

    # Process all clusters at once (if enough RAM/CPU):
    python process_pt_seq80.py --n_proc 32
"""
...
def save_cluster_pt(cluster_item, protein_chain_map):
    ...
    try:
        pyg_data = codnas_to_pyg(
            ...
            str(PDB_CACHE_DIR), protein_chain_map
        )
...
def parse_args():
    p = argparse.ArgumentParser(description="Featurise pdb_cache_seq80 → train_pt_multi_chain")
...
def main():
    ...
    # Skip clusters without a CIF (gather_sequences_seq80 may not have finished them)
    missing_cif = [
