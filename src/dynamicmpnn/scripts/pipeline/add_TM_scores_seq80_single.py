"""
Add TM-scores from foldseek_alignments to train_pt_single_chain/*.pt files.

This script:
1. Loads .pt files from train_pt_single_chain/
...
5. Saves updated files back to train_pt_single_chain/ (in-place update)

Clusters with no matching allvsall.tsv get a fallback 1x1 dummy matrix.

Usage (interactive):
    python add_TM_scores_seq80_single.py

For SLURM array jobs:
    sbatch slurm_add_TM_scores_seq80_single
"""
...
PDB_CACHE_DIR = PUBLIC_DB / "train_pt_single_chain"
OUTPUT_DIR    = PUBLIC_DB / "train_pt_single_chain"
...
def apply_fallback_and_save(data_object, filename, cluster_id, reason):
    ...
    try:
        ...
        # Filter pyg_dict to keep only selected members (match full chain name)
...
def update_single_file(filename):
    ...
    try:
        ...
        # --- 3. Build chain mapping ---
        # foldseek chains: lowercase pdb_id + original-case auth chain (e.g. "1abc_H", "1abc_h")
        # pt_members: UPPERCASE pdb_id + original-case auth chain (e.g. "1ABC_H", "1ABC_h")
        # Normalize PDB ID to lowercase for matching; chain ID stays case-sensitive
        def normalize_member(name):
            """Convert '5FGO_A' -> '5fgo_A' (lowercase PDB ID, preserve chain case)."""
            name = name.strip()
            if '_' in name:
                pdb_id, chain = name.rsplit('_', 1)
        ...
        for m in pt_members:
            m_clean = m.strip()
            m_normalized = normalize_member(m_clean)
            if m_normalized in chain_order_map:
                aligned_members.append((m, chain_order_map[m_normalized]))
            else:
                missing_from_foldseek.append(m_clean)

        pt_members_normalized = {normalize_member(m) for m in pt_members}
        extra_in_foldseek = [c for c in foldseek_chains if c not in pt_members_normalized]
...
def run_multiprocessing_update():
    ...
    logger.info(f"Task {task_id}/{num_tasks}: Starting TM-score addition (single-chain) from foldseek_alignments...")
    ...
    print("\n" + "=" * 50)
    logger.info(f"Task {task_id} Summary (single-chain)")
    print("=" * 50)
